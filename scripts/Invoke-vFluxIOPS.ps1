...
Begin {
    ...
    $InfluxStruct = New-Object -TypeName PSObject -Property @{
        InfluxDbServer            = $env:INFLUX_DB_SERVER            #IP Address,DNS Name, or 'localhost'
        InfluxDbPort              = $env:INFLUX_DB_PORT              #default for InfluxDB is 8086
        InfluxDbName              = $env:INFLUX_DB_NAME              #to follow my examples, set to 'compute' here and run "CREATE DATABASE compute" from Influx CLI
        InfluxDbUser              = $env:INFLUX_DB_USER              #to follow my examples, set to 'esx' here and run "CREATE USER esx WITH PASSWORD esx WITH ALL PRIVILEGES" from Influx CLI
        InfluxDbPassword          = $env:INFLUX_DB_PASSWORD          #to follow my examples, set to 'esx' [requires user pw set in Influx CLI or similar]
...
Process {
    ...
    ## Connect to vCenter
    try {
        $null = Connect-VIServer -Server $env:VMWARE_VCENTER_HOST -User $env:VMWARE_VCENTER_USER -Password $env:VMWARE_VCENTER_PASSWORD -WarningAction Continue -ErrorAction Stop
    }
    Catch {
